...
export const userExpenses = (request, response) => {
...
export const addExpenses = (request, response) => {
    response.setHeader('access-control-allow-origin', '*');

    const expense = JSON.parse(request.body);

    db.collection('expenses')
        .add({ 'isClosed': false, ...expense })
        .then(_ => {
            response.send();
        })
        .catch(error => {
...
export const groupExpenses = (request, response) => {
    response.setHeader('access-control-allow-origin', '*');
...
export const closeExpenses = (request, response) => {
    response.setHeader('access-control-allow-origin', '*');
